...
maxplanck = "/home/wilderwohns/adna_ts/max_planck_data/"
...
for chromosome in list(range(1, 23)) + ["X", "Y"]:
    ...
    tgp = pd.read_csv("1kg.GRCh37.chr" + str(chromosome) + ".snps.txt",
                      sep="\t", names=column_names, skiprows=1)
    try:
        datasets['sgdp'] = pd.read_csv(
                "sgdp.chr" + str(chromosome) + ".snps.pos.alleles.txt",
                sep="\t", names=column_names, skiprows=1)
    except:
    ...
    try:
        datasets['ukbb'] = pd.read_csv(
                "ukbb.chr" + str(chromosome) + ".snps.txt", sep=" ")
    except:
        logging.info("UKB phased sex chromosomes not available")
    reich_array = pd.read_csv(
            "reich1240karray.snps.pos.alleles.txt", sep=" ", header=None,
            names=column_names)
    ...
    datasets['hgdp'] = pd.read_csv(
            "hgdp_wgs.20190516.chr" + str(chromosome) + ".GRCh37.snps.txt",
            sep=" ", header=None, names=column_names)
    datasets['denisovan'] = pd.read_csv(
            maxplanck + "denisovan.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['altai'] = pd.read_csv(
            maxplanck + "altai.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['vindija'] = pd.read_csv(
            maxplanck + "vindija.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['ust_ishim'] = pd.read_csv(
            maxplanck + "ust_ishim.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['chagyrskaya'] = pd.read_csv(
            maxplanck + "chagyrskaya.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['lbk'] = pd.read_csv(
            maxplanck + "lbk.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
    datasets['loshbour'] = pd.read_csv(
            maxplanck + "loshbour.chr" + str(chromosome) + ".GRCh37.snps.txt",
            delimiter=" ", names=column_names, skiprows=1)
