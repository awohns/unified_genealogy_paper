...
base_filename = "data/hgdp_1kg_sgdp_high_cov_ancients_chr20.dated.20nodes_all.tmrcas"
...
histdata_tmrcas = np.load(
    base_filename + ".npz"
)  # Rows are arranged as [(0, 0), (0, 1), (0, 2), (0, 3), (1, 1), (1, 2), (1, 3), (2, 2), (2, 3), (3, 3)]
...
class LinkedView(plugins.PluginBase):
    ...
    def __init__(
        self, points, histlabel, histline, meanline, histdata, meandata, order
    ):
...
row_order = getattr(
    result.dendrogram_row, "reordered_ind", np.arange(mean_tmrca_df.shape[0])
)
col_order = getattr(
    result.dendrogram_col, "reordered_ind", np.arange(mean_tmrca_df.shape[1])
)
...
)  # put the histogram where the column dendrogram normally goes
...
x, y = np.append(bins, bins[-1]), np.pad(
    np.zeros(dist_tmrcas.shape[1]), 1
...
histogram_plot.plot(
    [0, 0], [0, 1], linewidth=4, color="w"
)  # Hide the mean line marker by making it white
